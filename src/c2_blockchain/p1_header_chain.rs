...
impl Header {
    ...
    fn verify_sub_chain(&self, chain: &[Header]) -> bool {
        ...
        // Check headers
        for (_, child_header) in chain.iter().enumerate() {
            // Check for the height
            if child_header.height != current_header.height + 1 {
                return false;
            }
            // Check for the parent hash
            if child_header.parent != hash(&current_header) {
                return false;
            }

            current_header = child_header.clone();
        }
